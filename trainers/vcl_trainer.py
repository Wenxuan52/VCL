...
def save_task_samples(model, task_id, digit_for_head, out_dir, device, sample_theta=False):
    ...
    with torch.no_grad():
        ...
        imgs = model.decode(digit_for_head, z, sample_theta=sample_theta, reshape=True)
...
def train_task(
    ...
    # TF1 onlinevi update_q_sigma behavior for shared decoder posterior.
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in loader:
            ...
            # Match TF1 onlinevi: regularize shared decoder only.
            param_kl = kl_module_to_snapshot(model.shared, shared_snapshot)

            # TF1 construct_optimizer:
            # loss = -mean(lowerbound) + KL_theta / N_task_data
            #      = mean(recon + klz) + KL_theta / N_task_data
...
if __name__ == "__main__":
    ...
    parser.add_argument("--log_sigma_init", type=float, default=-6.0)
    parser.add_argument("--device", type=str, default=None)
    parser.add_argument("--sample_theta", action="store_true", help="Enable theta sampling")
    parser.add_argument("--no_sample_theta", action="store_true", help="Disable theta sampling")

    args = parser.parse_args()

    sample_theta = True
    if args.no_sample_theta:
        sample_theta = False
    elif args.sample_theta:
        sample_theta = True
